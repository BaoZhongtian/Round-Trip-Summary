...
if __name__ == '__main__':
    ...
    train_loader, val_loader, test_loader = loader_cnndm(
        batch_size=9, tokenizer=tokenizer, keywords_name='SalientWords')
    model = BartForCausalLM.from_pretrained('C:/PythonProject/bart-base')
    ...
    for episode_index in range(episode_number):
        for batch_index, batch_article in enumerate(train_loader):
            # if (episode_index * len(train_loader) + batch_index) < 13999: continue
