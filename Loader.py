...
def loader_cnndm(batch_size=4):
    ...
    val_data = json.load(open(os.path.join(load_path, 'CNNDM_val.json'), 'r'))[0:100]
    test_data = json.load(open(os.path.join(load_path, 'CNNDM_test.json'), 'r'))[0:100]
    ...
    val_dataset = DataLoader(val_data, batch_size=batch_size, shuffle=False, collate_fn=collate)
